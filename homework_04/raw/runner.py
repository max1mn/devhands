...
def run_wrk_helper(args: Namespace):
    ...
    if len(args.c) > 1:
        connections = ",".join(map(str, args.c))
...
def run_wrk_parser():
    ...
    reports = list(filter(lambda x: x.endswith(".txt"), files))

    for report in reports:
        Process = Popen(
            [f"python ./wrk_parser.py {report}"], shell=True, stdin=PIPE, stderr=PIPE
        )
        Process.communicate()
...
def main():
    ...
    parser.add_argument(
        "-t", type=int, help="Number of threads (default: $DEFAULT_THREADS)"
    )

    parser.add_argument("-d", type=int, help="Duration")

    parser.add_argument("-c", type=int, nargs="+", help="connections, eg -c 512 1024")
